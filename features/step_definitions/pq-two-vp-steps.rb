When(/^Filter "(.*?)" from Media Type Overlay$/) do | media_type |
  ...
  find('.overlay-close-btn.top').click
end
...
Then(/^I should see both videos and photos on the page$/) do
  all('.media-container-details').each { | asset | asset.find(asset.first('a')["class"] == 'grid-photo-link' ? '.media-gallery-icon' : '.media-play-icon') }
end